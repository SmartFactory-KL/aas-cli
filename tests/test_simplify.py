...
from aas_cli.simplify import simplify, simplify_many


def test_simplify(workpiece_carrier_a1):
    shell = api.get_shell(workpiece_carrier_a1.id)
...
def test_simplify_submodel(workpiece_carrier_a1):
    submodel = api.get_submodel(workpiece_carrier_a1.good_information.id)
